...
echo "[$(date -Is)] ensure_recent_etf_share_data: share_latest=${LATEST_SHARE_DATE:-none} agg_latest=${LATEST_AGG_DATE:-none} target=$TARGET_SH yesterday=$YESTERDAY_SH today=$TODAY_SH"

SHOULD_BACKFILL=0
if [[ -z "$LATEST_SHARE_DATE" || "$LATEST_SHARE_DATE" -lt "$TARGET_SH" ]]; then
  SHOULD_BACKFILL=1
fi
if [[ -z "$LATEST_AGG_DATE" || "$LATEST_AGG_DATE" -lt "$TARGET_SH" ]]; then
  SHOULD_BACKFILL=1
fi

if [[ "$SHOULD_BACKFILL" == "1" ]]; then
  echo "[$(date -Is)] ensure_recent_etf_share_data: ETF data not caught up to Beijing target date, run targeted backfill"
...
echo "[$(date -Is)] ensure_recent_etf_share_data: final share_latest=${LATEST_SHARE_DATE:-none} agg_latest=${LATEST_AGG_DATE:-none} target=$TARGET_SH"
