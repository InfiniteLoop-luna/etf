...
- 🕒 **股票分时优先级**: 股票 `1min` 分时数据优先使用 `mootdx`，失败时自动回退 `Tushare`
- 🧩 **自选基金对比矩阵**: 在“自选基金 → 视图模式 → 对比矩阵”中选择至少两只基金，可切换股票持仓、行业分布和共同持股数量，并筛选共同持仓或仅单只基金出现的股票。比较复用本页已加载的股票持仓样本，权重口径为占基金股票资产比例；缺失数据、未观察到的持仓和不同报告期会明确标注。
...
项目已接入 `mootdx` 用于股票分时数据展示：

- 股票 `1min` 分时数据优先使用 `mootdx`
- 当 `mootdx` 不可用、无数据或市场不支持时，会自动回退到 `Tushare`
- 已写入数据库的分时数据会优先复用缓存
...
如运行环境临时缺少 `mootdx`，应用仍会保留原有 `Tushare` 回退能力，不会导致整个页面不可用。
