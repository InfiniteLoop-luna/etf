...
from src.etf_stats import (
    ...
    get_industry_index_available_dates, get_industry_index_timeseries,
    get_industry_index_constituent_detail,
...
.ws-fund-watchboard__metric.is-up strong {
    color:var(--fw-red) !important;
    -webkit-text-fill-color:var(--fw-red) !important;
}
.ws-fund-watchboard__metric.is-down strong {
    color:var(--fw-green) !important;
    -webkit-text-fill-color:var(--fw-green) !important;
}
...
.ws-fund-watchboard__confirmed-nav > div {
    min-width:0;
    padding:.12rem .2rem;
}
...
.ws-fund-watchboard__card-metrics .is-up strong {
    color:var(--fw-red) !important;
    -webkit-text-fill-color:var(--fw-red) !important;
}
.ws-fund-watchboard__card-metrics .is-down strong {
    color:var(--fw-green) !important;
    -webkit-text-fill-color:var(--fw-green) !important;
}
...
.st-key-fund_watchlist_card_grid [data-testid="stColumn"] {
...
.st-key-fund_watchlist_add_panel [data-testid="stCaptionContainer"] p {
...
.st-key-fund_watchlist_add_panel .stTextInput input::placeholder {
    ...
    -webkit-text-fill-color:#6c7c93 !important;
    opacity:1 !important;
}
.st-key-fund_watchlist_add_panel [data-baseweb="select"] > div *,
.st-key-fund_watchlist_add_panel [data-baseweb="select"] span {
    color:#13213b !important;
    -webkit-text-fill-color:#13213b !important;
}
.st-key-fund_watchlist_add_panel [data-testid="stButton"] > button,
.st-key-fund_watchlist_add_panel [data-testid="stFormSubmitButton"] > button {
...
.st-key-fund_watchlist_add_panel [data-testid="stButton"] > button:disabled,
.st-key-fund_watchlist_add_panel [data-testid="stFormSubmitButton"] > button:disabled {
    border-color:rgba(151,177,214,.35) !important;
    background:#314460 !important;
    color:#d7e3f2 !important;
    -webkit-text-fill-color:#d7e3f2 !important;
...
.st-key-fund_watchlist_table_focus_controls {
    ...
    padding:.72rem .82rem;
    border:1px solid rgba(70,126,255,.32);
    border-radius:10px;
    background:linear-gradient(145deg,rgba(8,26,57,.94),rgba(3,12,30,.96));
}
...
.st-key-fund_watchlist_toolbar [data-testid="stCaptionContainer"] p,
.st-key-fund_watchlist_table_batch_controls label,
.st-key-fund_watchlist_table_batch_controls label p,
.st-key-fund_watchlist_table_batch_controls label *,
.st-key-fund_watchlist_table_focus_controls label,
.st-key-fund_watchlist_table_focus_controls label p,
.st-key-fund_watchlist_table_focus_controls label * {
    color:#e6effa !important;
    -webkit-text-fill-color:#e6effa !important;
}
...
.st-key-fund_watchlist_toolbar [data-baseweb="select"] > div,
.st-key-fund_watchlist_table_batch_controls [data-baseweb="select"] > div,
.st-key-fund_watchlist_table_focus_controls [data-baseweb="select"] > div {
    border-color:rgba(151,188,255,.72) !important;
    background:#f7faff !important;
    color:#13213b !important;
    -webkit-text-fill-color:#13213b !important;
...
def _render_application_page() -> PageStatus:
    ...
    if iphone_mode:
        ...
        if mobile_group == "Favorite":
            ...
            elif mobile_page == ETF_INDUSTRY_PAGE_LABEL:
                render_industry_etf_tab()
    ...
    if selected_module == decision_module_label:
        ...
        if selected_page == ETF_MAIN_PAGE_LABEL:
            ...
            render_wide_index_tab()
        elif selected_page == ETF_INDUSTRY_PAGE_LABEL:
            render_industry_etf_tab()
...
def render_industry_etf_tab():
    st.subheader("🏭 行业ETF总览")
    st.caption("展示按具体行业/主题指数聚合后的 ETF 总份额、总规模及较前一日变动，不影响现有宽基指数ETF页面")

    try:
        available = get_industry_index_available_dates(limit=2000)
    except Exception as e:
        st.error(f"获取行业ETF聚合日期失败: {e}")
        return

    if not available:
        st.warning("暂无行业ETF聚合数据，请先运行聚合脚本")
        return
    ...
    control_cols = st.columns([1.25, 1.15, 1.4, 1.0])
    with control_cols[0]:
        date_range = st.slider(
            "时间范围",
            min_value=min_d,
            ...
            value=(default_start, max_d),
            format="YYYY-MM-DD",
            key="industry_etf_date_range"
        )

    try:
        base_df = get_industry_index_timeseries(
            start_date=str(date_range[0]),
            end_date=str(date_range[1])
        )
    except Exception as e:
        st.error(f"查询行业ETF聚合数据失败: {e}")
        return

    if base_df is None or len(base_df) == 0:
        st.warning("所选时间范围内暂无行业ETF数据")
        return

    base_df = base_df.copy()
    base_df["trade_date"] = pd.to_datetime(base_df["trade_date"])
    latest_snapshot_date = base_df["trade_date"].max()
    latest_snapshot = (
        base_df[base_df["trade_date"] == latest_snapshot_date]
        .sort_values(["total_size_yi", "benchmark_index_name"], ascending=[False, True])
        .copy()
    )
    ...
    with control_cols[1]:
        keyword = st.text_input(
            "指数筛选",
            value="",
            placeholder="如：影视、证券、人工智能",
            key="industry_etf_keyword"
        ).strip()

    options_df = latest_snapshot.copy()
    if keyword:
        keyword_mask = (
            options_df["benchmark_index_name"].fillna("").astype(str).str.contains(keyword, case=False, na=False)
            | options_df["benchmark_index_code"].fillna("").astype(str).str.contains(keyword, case=False, na=False)
        )
        options_df = options_df[keyword_mask].copy()

    if options_df.empty:
        st.warning("当前筛选条件下暂无行业指数可选")
        return

    code_name_df = (
        options_df[["benchmark_index_code", "benchmark_index_name"]]
        .drop_duplicates()
        .sort_values(["benchmark_index_name", "benchmark_index_code"])
    )
    name_to_code = dict(zip(code_name_df["benchmark_index_name"], code_name_df["benchmark_index_code"]))
    name_options = code_name_df["benchmark_index_name"].tolist()
    default_names = name_options[: min(6, len(name_options))]
    ...
    with control_cols[3]:
        metric = st.radio(
            ...
            options=["总份额(亿份)", "总规模(亿元)"],
            index=0,
            key="industry_etf_metric"
        )

    if not selected_names:
        st.info("请至少选择一个行业主题指数")
        return

    selected_codes = [name_to_code[name] for name in selected_names if name in name_to_code]
    ts_df = base_df[base_df["benchmark_index_code"].isin(selected_codes)].copy()
    if ts_df.empty:
        st.warning("当前筛选条件下暂无数据")
        return
    ...
    with kpi_cols[0]:
        st.metric("最新交易日", latest_date.strftime('%Y-%m-%d'))
    with kpi_cols[1]:
        st.metric("选中指数数", f"{latest_df['benchmark_index_code'].nunique()}")
    with kpi_cols[2]:
        st.metric(
            "总份额(亿份)",
            f"{float(share_total):,.2f}" if pd.notna(share_total) else "-",
    ...
    if not size_change_chart_df.empty:
        st.caption("纵轴展示按当日收盘价 × 份额变化数计算的规模变动金额，hover 可查看变动比例")
        industry_change_view = st.radio(
            "规模变动展示方式",
            options=["曲线", "红绿柱状"],
            key="industry_etf_size_change_view",
            horizontal=True
        )
        size_change_fig = create_change_curve_chart(
            df=size_change_chart_df,
            value_col='size_change_yi',
            title='行业ETF 规模变动(亿元)趋势',
            yaxis_title='规模变动(亿元)',
            pct_col='size_change_pct',
            series_col='benchmark_index_name',
            series_names=selected_names,
        ...
        size_change_bar_fig = create_change_bar_chart(
            df=size_change_chart_df,
            value_col='size_change_yi',
            title='行业ETF 规模变动(亿元)红绿柱状图',
            yaxis_title='规模变动(亿元)',
            pct_col='size_change_pct',
            series_col='benchmark_index_name',
            series_names=selected_names
        )
        if industry_change_view == "曲线":
            st.plotly_chart(size_change_fig, use_container_width=True)
        else:
            st.plotly_chart(size_change_bar_fig, use_container_width=True)

    st.subheader("📋 最新行业指数排名")
    latest_rank_df = (
        latest_df.sort_values(['total_size_yi', 'benchmark_index_name'], ascending=[False, True]).copy()
    )
    latest_rank_df['ETF只数'] = latest_rank_df['etf_count'].fillna(0).astype(int)
    st.dataframe(
        latest_rank_df[
            ['benchmark_index_name', 'benchmark_index_code', 'ETF只数',
             'total_share_yi', 'share_change_yi', 'share_change_pct',
             'total_size_yi', 'size_change_yi', 'size_change_pct']
        ].rename(columns={
            'benchmark_index_name': '行业主题指数',
            'benchmark_index_code': '基准指数代码',
            'total_share_yi': '总份额(亿份)',
            'share_change_yi': '份额变动(亿份)',
            'share_change_pct': '份额变动比例(%)',
            'total_size_yi': '总规模(亿元)',
            'size_change_yi': '规模变动(亿元)',
            'size_change_pct': '规模变动比例(%)',
        }).style.format({
            '总份额(亿份)': '{:,.2f}',
            '份额变动(亿份)': '{:,.2f}',
            '份额变动比例(%)': '{:,.2f}',
    ...
    st.subheader("🧩 指数下ETF明细")
    focus_name = st.selectbox(
        "查看某个行业指数对应的ETF",
        options=selected_names,
        key="industry_etf_focus_name"
    )
    focus_code = name_to_code.get(focus_name)
    if focus_code:
        try:
            detail_df = get_industry_index_constituent_detail(
                trade_date=latest_date.strftime('%Y-%m-%d'),
                benchmark_index_code=focus_code
            )
        except Exception as e:
            st.error(f"查询行业ETF明细失败: {e}")
            detail_df = pd.DataFrame()

        if detail_df.empty:
            st.info("该指数在最新交易日暂无ETF明细")
        else:
            detail_df = detail_df.copy()
            detail_df['总份额(亿份)'] = pd.to_numeric(detail_df['total_share_yi'], errors='coerce')
            detail_df['总规模(亿元)'] = pd.to_numeric(detail_df['total_size_yi'], errors='coerce')
            detail_df['净值'] = pd.to_numeric(detail_df['nav'], errors='coerce')
            detail_df['收盘价'] = pd.to_numeric(detail_df['close'], errors='coerce')
            st.caption(f"{latest_date.strftime('%Y-%m-%d')} | {focus_name} | 共 {len(detail_df)} 只ETF")
            st.dataframe(
                detail_df[
                    ['ts_code', 'etf_name', 'exchange', '总份额(亿份)', '总规模(亿元)', '净值', '收盘价']
                ].rename(columns={
                    'ts_code': 'ETF代码',
                    'etf_name': 'ETF名称',
                    'exchange': '交易所',
    ...
    display_df = ts_df.sort_values(['trade_date', 'benchmark_index_code'], ascending=[False, True]).copy()
    display_df['日期'] = display_df['trade_date'].dt.strftime('%Y-%m-%d')
    display_df['行业主题指数'] = display_df['benchmark_index_name']
    display_df['ETF只数'] = display_df['etf_count'].fillna(0).astype(int)
    display_df['总份额(亿份)'] = pd.to_numeric(display_df['total_share_yi'], errors='coerce')
    display_df['份额变动(亿份)'] = pd.to_numeric(display_df['share_change_yi'], errors='coerce')
    display_df['份额变动比例(%)'] = pd.to_numeric(display_df['share_change_pct'], errors='coerce')
    ...
    st.dataframe(
        display_df[
            ['日期', '行业主题指数', 'benchmark_index_code', 'ETF只数',
             '总份额(亿份)', '份额变动(亿份)', '份额变动比例(%)',
             '总规模(亿元)', '规模变动(亿元)', '规模变动比例(%)']
        ].rename(columns={'benchmark_index_code': '基准指数代码'}).style.format({
            '总份额(亿份)': '{:,.2f}',
...
@st.cache_data(ttl=FUND_WATCHLIST_INTRADAY_CACHE_TTL_SECONDS, show_spinner=False)
def load_fund_watchlist_latest_closing_estimates_cached(
    fund_codes: tuple[str, ...],
) -> dict[str, dict]:
    from src.fund_estimate_snapshot_store import (
        ensure_fund_estimate_snapshot_table,
...
def load_fund_watchlist_dashboard_data(
    ...
    try:
        ensure_fund_estimate_snapshot_table(fund_engine)
    except Exception as exc:
    ...
    for _, watchlist_row in watchlist_df.iterrows():
        ...
        if estimate_store_error:
            ...
            try:
                ...
                logger.warning(
                    "fund watchlist estimate snapshot load failed for %s / %s: %s",
                    fund_code,
                    nav_date.date(),
                    exc,
                )
...
def _fund_watchlist_money_label(value, *, fallback: str = "--") -> str:
    number = pd.to_numeric(value, errors="coerce")
    if pd.isna(number):
        return fallback
...
def _fund_watchlist_signed_point_label(value, *, fallback: str = "--") -> str:
    number = pd.to_numeric(value, errors="coerce")
    if pd.isna(number):
        return fallback
    return f"{float(number):+.2f}pp"
...
def render_fund_watchlist_summary(summary: dict) -> None:
    ...
    estimated_amount = summary.get("estimated_daily_amount")
    ...
    estimated_amount_date_label = _fund_watchlist_date_label(
        summary.get("estimated_daily_amount_date")
    )
...
def _build_fund_watchlist_card_html(item: dict, focus_code: str) -> str:
    ...
    estimated_amount_source = _fund_watchlist_text(
        item.get("estimated_daily_amount_source"), "等待有效估值"
    )
    daily_change = item.get("daily_change_pct")
    daily_change_label = _fund_watchlist_signed_pct_label(daily_change)
    daily_change_tone = _fund_watchlist_intraday_tone(daily_change)
    closing_estimate = item.get("closing_estimate_pct")
    closing_estimate_label = _fund_watchlist_signed_pct_label(closing_estimate)
    closing_estimate_tone = _fund_watchlist_intraday_tone(closing_estimate)
    estimate_deviation = item.get("estimate_deviation_pct")
    estimate_deviation_label = _fund_watchlist_signed_point_label(estimate_deviation)
...
def render_fund_watchlist_table(items: list[dict], *, focus_code: str) -> str:
    ...
    if is_batch_mode:
        with st.container(key="fund_watchlist_table_batch_controls"):
    ...
    with st.container(key="fund_watchlist_table_wrap"):
        ...
        st.dataframe(
            ...
            column_config={
                ...
                "估值偏差(百分点)": st.column_config.NumberColumn(format="%+.2f"),
    ...
    with st.container(key="fund_watchlist_table_focus_controls"):
...
def render_fund_watchlist_focus_detail(item: dict) -> None:
    ...
    daily_change_class = f' class="{daily_change_tone}"' if daily_change_tone else ""
    closing_estimate = item.get("closing_estimate_pct")
    closing_estimate_label = _fund_watchlist_signed_pct_label(closing_estimate)
    closing_estimate_tone = _fund_watchlist_intraday_tone(closing_estimate).strip()
    closing_estimate_class = f' class="{closing_estimate_tone}"' if closing_estimate_tone else ""
    estimate_deviation = item.get("estimate_deviation_pct")
    estimate_deviation_label = _fund_watchlist_signed_point_label(estimate_deviation)
    estimate_deviation_tone = _fund_watchlist_intraday_tone(estimate_deviation).strip()
    estimate_deviation_class = f' class="{estimate_deviation_tone}"' if estimate_deviation_tone else ""
    ...
    with st.container(key="fund_watchlist_add_panel"):
            ...
            with st.form("fund_watchlist_manual_position_form", border=False):
                ...
                with manual_cols[0]:
                    manual_code = st.text_input(
                        "基金代码",
                        placeholder="如 001938、510300.SH",
                        key="fund_watchlist_manual_code",
                    ).strip()
                with manual_cols[1]:
                    manual_shares_text = st.text_input(
                        "持有份额",
                        placeholder="如 12,345.67 或 1.2万",
            ...
            if manual_submitted:
                ...
                if not manual_code:
                    ...
                    try:
                        ...
                        matched = choose_unique_fund_match(manual_code, matches)
                        if not matched:
                            st.warning("无法唯一确认该基金代码，请补充 .OF、.SH 或 .SZ 后缀。")
                        else:
                            fund_code = str(matched.get("fund_code") or "").strip().upper()
                            fund_name = str(matched.get("name") or fund_code).strip() or fund_code
                            add_watchlist_item(
                                current_username,
                                fund_code,
            ...
            if not candidates.empty:
                ...
                with add_cols[0]:
                    if st.button(
                        ...
                        searched_shares = (
                            parse_share_amount(searched_shares_text)
        ...
        with screenshot_tab:
            ...
            if isinstance(saved_ocr_preview, dict) and (
                ...
                or saved_ocr_preview.get("image_fingerprint")
            ...
            if (
                ...
                and ocr_payload.get("username") == current_username
                ...
                if st.button(
                    "确认导入勾选持仓",
                    key="fund_watchlist_ocr_confirm",
                    type="primary",
                    use_container_width=True,
                ):
                    selected_preview = edited_preview[
                        edited_preview["导入"].fillna(False).astype(bool)
                    ]
                    if selected_preview.empty:
                        st.warning("请至少勾选一条要导入的持仓。")
                    else:
                        resolved_positions = []
                        validation_errors = []
...
def render_fund_watchlist_live_dashboard(items: list[dict], current_username: str) -> None:
    ...
    try:
        ...
        dashboard_items = [
            attach_latest_closing_estimate(
                item,
                latest_estimate_map.get(item["fund_code"]),
            )
            for item in items
        ]
    except Exception as exc:
        logger.warning("fund watchlist latest closing estimate load failed: %s", exc)

    quotes = {}
    quote_error = ""
    if market_state["is_active"]:
        symbols = collect_fund_holding_symbols(dashboard_items)
        if symbols:
            try:
    ...
    st.caption(
        "15:00估值与实际净值同日对比：每日估值偏差 = 同一净值日期的 15:00 估值涨跌幅－实际涨跌幅；"
        "也就是估值偏差（估值－实际）；未同日匹配时显示 --。"
    )
    st.caption(
    ...
    render_fund_watchlist_summary(
        build_fund_watchlist_summary(
            live_items,
            target_date=market_state["market_date"],
        )
    )
