...
class NavigationConfigTests(unittest.TestCase):
    ...
    def test_style_historical_st_badge_column_uses_stripi_cream_palette(self):
        styles = style_historical_st_badge_column(pd.Series([HISTORICAL_ST_BADGE_TEXT, "", None]))

        self.assertIn("background-color: #F5E9D4", styles[0])
        self.assertIn("color: #0D253D", styles[0])
